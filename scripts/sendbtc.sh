...
"$BUILD_DIR/$BIN_NAME" mpcsendbtc "$SERVER" "$SESSION_ID" "$PARTY1" "$PARTIES" "$PUBLIC_KEY2" "$PRIVATE_KEY1" "$KEYSHARE1" "$DERIVATION_PATH" "$RECEIVER_ADDRESS" "$AMOUNT_SATOSHI" "$ESTIMATED_FEE" &
PID1=$!

echo "Starting mpcsendbtc for PARTY2..."
"$BUILD_DIR/$BIN_NAME" mpcsendbtc "$SERVER" "$SESSION_ID" "$PARTY2" "$PARTIES" "$PUBLIC_KEY1" "$PRIVATE_KEY2" "$KEYSHARE2" "$DERIVATION_PATH" "$RECEIVER_ADDRESS" "$AMOUNT_SATOSHI" "$ESTIMATED_FEE" &
PID2=$!
